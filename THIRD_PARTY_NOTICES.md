...
| qpdf 12.3.2 | Bundled Windows executable and DLLs in `plugins/qpdf/` and used for PDF linearization / version rewrite | `Apache-2.0` | https://qpdf.sourceforge.io/ , https://github.com/qpdf/qpdf |
...
1. 本仓库当前直接包含 `qpdf 12.3.2` 的 Windows 可执行文件与配套 DLL。对外分发包含这些文件的源码包或二进制包时，应同时保留本文件与根目录 `LICENSE`。
2. 若你分发了包含 `PyMuPDF`、`qpdf` 或其本地修改版本的对象代码、冻结打包产物或安装包，应确保接收方可获得对应版本的源码以及你对这些组件所做的修改。
